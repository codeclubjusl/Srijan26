    ...
    >
            ...
            {msg.role === "assistant" && (
              <img
                ...
                style={{
                  ...
                  marginTop: 2, border: "1px solid rgba(235,216,125,0.25)",
                  background: "#2a0d0d", objectFit: "cover",
                }}
                ...
                style={
                  msg.role === "user"
                    ? {
                        ...
                        background: "linear-gradient(135deg, #f09400, #ebd87d)",
                        color: "#160505", fontWeight: 600, lineHeight: 1.5,
                        wordBreak: "break-word", boxShadow: "0 4px 16px rgba(240,148,0,0.25)",
                      }
                    : {
                        padding: "10px 14px", fontSize: 13,
                ...
                {(msg.link || (msg.drive_link && msg.drive_link !== "None")) && (
                  <div
                    style={{
                      ...
                      borderTop: "1px solid rgba(235,216,125,0.1)",
                      display: "flex", flexWrap: "wrap", gap: 8,
                    }}
                  >
                    {msg.link && (
                      <a
                        href={`https://srijanju.in${msg.link}`}
                        target="_blank" rel="noopener noreferrer"
                        style={{
                          display: "inline-flex", alignItems: "center", gap: 5,
                          fontSize: 11, fontWeight: 700, padding: "5px 12px", borderRadius: 6,
                          background: "linear-gradient(135deg, rgba(240,148,0,0.2), rgba(235,216,125,0.15))",
                          border: "1px solid rgba(240,148,0,0.35)", color: "#ebd87d",
      ...
      {messages.length === 1 && !isLoading && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingLeft: 32 }}>
          {QUICK_QUESTIONS.map((q, i) => (
            <button
              key={q.id}
              className="srijan-chip"
              style={{
                animationDelay: `${i * 0.07}s`, fontSize: 11, padding: "6px 13px",
                borderRadius: 99, border: "1px solid rgba(235,216,125,0.2)",
                background: "rgba(235,216,125,0.05)", color: "rgba(235,216,125,0.8)",
                cursor: "pointer", textAlign: "left", fontFamily: "inherit",
                letterSpacing: "0.02em", transition: "all 0.15s",
              }}
              onMouseEnter={(e) => {
                const b = e.currentTarget as HTMLButtonElement;
                b.style.background = "rgba(235,216,125,0.12)";
                b.style.borderColor = "rgba(235,216,125,0.4)";
                b.style.color = "#ebd87d";
                b.style.transform = "scale(1.04)";
              }}
              onMouseLeave={(e) => {
                const b = e.currentTarget as HTMLButtonElement;
                b.style.background = "rgba(235,216,125,0.05)";
                b.style.borderColor = "rgba(235,216,125,0.2)";
                b.style.color = "rgba(235,216,125,0.8)";
      ...
      {isLoading && (
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <img
            src={BOT_AVATAR} alt=""
            style={{ width: 24, height: 24, borderRadius: "50%", opacity: 0.4, filter: "grayscale(1)", objectFit: "cover" }}
          />
          <div
            style={{
              padding: "8px 14px", borderRadius: "4px 18px 18px 18px",
              background: "rgba(255,255,255,0.04)", border: "1px solid rgba(235,216,125,0.1)",
              display: "flex", alignItems: "center", gap: 8,
            }}
          >
            <Loader2 size={13} style={{ color: "#f09400" }} className="animate-spin" />
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", letterSpacing: "0.05em" }}>
              Thinking…
            </span>
          </div>
        </div>
